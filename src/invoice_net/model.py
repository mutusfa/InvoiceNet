...
class InvoiceNet(InvoiceNetInterface):
    def create_model(self, data_handler, config):
        ...
        words = Embedding(
            data_handler.embeddings.shape[0],
...
class InvoiceNetCloudScan(InvoiceNetInterface):
    def create_model(self, data_handler, config):
        ...
        features = Input(
            shape=(num_features * 5,), dtype="float32", name="features"
        )
    ...
    def prepare_data(self, features, labels):
        ...
        spatial_features = np.zeros(
            [features_array.shape[0], features_array.shape[1] * 4],
            dtype=np.float32,
        )
        ...
        for i in range(features_array.shape[0]):
            vectors = [
                zero_vec if j == -1 else features_array[j]
        ...
        features_array = np.concatenate(
            (features_array, spatial_features), axis=1
        )
